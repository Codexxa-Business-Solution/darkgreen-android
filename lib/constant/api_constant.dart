
class ApiConstants {

  String baseUrl = "https://darkgreen.in/app-admin/api-firebase/";

  String accessKey = "90336";

  String sliderImageUrl = "https://darkgreen.in/app-admin/upload/slider/";

  String token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpYXQiOjE2ODAyNjM2MjksImlzcyI6ImVLYXJ0IiwiZXhwIjoxNjgwMjY1NDI5LCJzdWIiOiJlS2FydCBBdXRoZW50aWNhdGlvbiIsIndlYiI6IjI5MjYwNDE4IiwiY3VzdCI6IjIyMDE1OTExIiwiYWRtaW4iOiIzMDI4ODQ0MiJ9.RHRoA1tVqfta3EnOq6JhpewCw2J4yFWOOxFO0zdpoZk";

  String usersRegister = "user-registration.php";
  String usersLogin = "login.php";
  String offerImages = "offer-images.php";
  String sliderImages = "slider-images.php";
  String allCategories = "get-categories.php";
  String subCategories = "get-subcategories-by-category-id.php";
  String productBySubCategories = "get-products-by-subcategory-id.php";
  String addToCart = "cart.php";
  String addFavorite = "favorites.php";
  String searchProduct = "products-search.php";
  String getProductInfoById = "get-product-by-id.php";
  String getSimilarProductById = "get-similar-products.php";
  String getAddUserAddress = "user-addresses.php";



}